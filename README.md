...
## Proje Özeti
Bu proje, makine öğrenimi ve derin öğrenme teknikleri kullanılarak hisse senedi fiyatlarının (Zaman Serisi) tahmin edilmesini amaçlamaktadır. Proje kapsamında PyTorch framework'ü kullanılarak LSTM (Long Short-Term Memory) ve GRU (Gated Recurrent Unit) olmak üzere iki farklı tekrarlayan sinir ağı (RNN) modeli inşa edilmiş ve performansları karşılaştırılmıştır.
...
Her iki model de aynı eğitim verisi üzerinde 100 epoch boyunca eğitilmiş olup, görünmeyen test verisi üzerindeki performansları (MSE / RMSE metrikleri) ve eğitim süreleri aşağıdaki gibidir:

*   **LSTM Modeli:** MSE: 34.87 | RMSE: 5.91 | Eğitim Süresi: ~52 saniye
*   **GRU Modeli:** MSE: 30.78 | RMSE: 5.55 | Eğitim Süresi: ~34 saniye

**Sonuç:** Beklenildiği üzere GRU modeli, sahip olduğu daha az parametre mimarisi sayesinde eğitimi çok daha hızlı tamamlamış ve hata metriklerinde (MSE/RMSE) LSTM'den daha isabetli bir performans sergilemiştir.
